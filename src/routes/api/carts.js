...
router.get('/:cartId([0-9a-fA-F]{24})', asyncHandler(async (req, res) => {
    const cart = await cartManager.getCartById(req.params.cartId);
    res.status(200).json({ status: 'success', payload: cart });
}));

router.put('/:cartId([0-9a-fA-F]{24})', asyncHandler(async (req, res) => {
...
router.delete('/:cartId([0-9a-fA-F]{24})', asyncHandler(async (req, res) => {
