...
let cartManagerInstance = new CartManager(CARTFILENAME)
...
//We make a small cart
let obj = productManagerInstance.getProducts()
let cartProductArray = obj.products.map( p => {
    return {product:p.id, quantity:1}
})
cartManagerInstance.addCart({productsArray: cartProductArray})
...
//We make a small cart
let obj2 = productManagerInstance.getProducts()
cartProductArray = obj2.products.map( p => {
    return {product:p.id, quantity:1}
})
cartManagerInstance.addCart({productsArray: cartProductArray})